...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 `xcodegen generate && xcodebuild build ...` 같은 실제 실행 가능한 검증 커맨드를 포함한다.
...
xcodebuild build -project SpamCall070.xcodeproj -scheme SpamCall070 -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO   # 빌드 에러 없음
```
